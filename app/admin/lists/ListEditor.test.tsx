...
describe('ListEditor — 찾기·거르기·페이지', () => {
  ...
  it('한 화면에 다 쏟지 않고 끊어서 보여준다', () => {
    const { container } = render(<ListEditor config={ADMIN_LISTS.timeline} initialRows={timeline(20)} />);
  ...
  it('검색어로 거른다', async () => {
    const user = userEvent.setup();
    render(<ListEditor config={ADMIN_LISTS.timeline} initialRows={timeline(20)} />);
  ...
  it('모아 보기는 보기만 바꾸고 저장 순서는 건드리지 않는다', async () => {
    const user = userEvent.setup();
    const { container } = render(<ListEditor config={ADMIN_LISTS.timeline} initialRows={timeline(12)} />);
  ...
  it('모아 보기 중에는 ↑↓ 를 잠근다 — 보이는 자리와 저장되는 자리가 다르다', async () => {
    const user = userEvent.setup();
    render(<ListEditor config={ADMIN_LISTS.timeline} initialRows={timeline(12)} />);
    await user.click(screen.getByRole('button', { name: /상태별로 모아 보기/ }));

...
**방문자 대시보드:** `/admin` 허브 상단에 **오늘 방문자 · 총 방문자 · 최근 7일 추세**가 표시됩니다. 공개 페이지 로드 시 익명 쿠키(`nz_vid`) 기반 비콘이 `/api/visit` → `record_visit`로 순 방문자를 기록하고(하루 1인 1회, IP·개인정보 미저장, 관리자 방문 제외), 통계는 로그인 관리자만 `get_visit_stats`로 조회합니다. `0005` 마이그레이션 미적용 시 "집계 준비 중"으로 표시됩니다.

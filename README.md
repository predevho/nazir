...
참고: 사진 업로드는 Phase 3C에서 추가됩니다(현재 인물·참여자 사진 자리는 placeholder).
...
- 주요 등장인물은 이름·설명만 편집합니다(사진은 저장 시 보존, 업로드는 3C).

**참여자 명단 편집 (Phase 3B-3b):** `/admin` 허브 → **참여자 명단**(`/admin/lists/people`)에서 그룹(헤더진·팀원·배우 등)과 각 개인(역할·이름·약력)을 추가/수정/삭제/순서변경 후 저장하면 즉시 반영됩니다. 그룹 삭제 시 소속 개인도 함께 삭제됩니다.

- 사진 업로드는 다음 단계(3C)에서 추가됩니다(현재 사진 자리는 placeholder, 저장 시 보존).

**약력 마크다운 (Phase 3B-4):** 참여자 약력과 주요 등장인물 설명은 **마크다운**으로 작성합니다. 불릿(`- 항목`), 강조(`**굵게**`), 링크 등이 공개 페이지에 서식대로 표시됩니다(경력 리스트 등 긴 소개에 적합). 관리자 입력칸은 넓은 여러 줄 텍스트 영역입니다. (`components/MarkdownText.tsx`, `react-markdown`)
...
- **Phase 3B-4 (완료)** — 약력·설명 마크다운 작성·표시
